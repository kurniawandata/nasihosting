...
echo " Version 1.0 Beta 29 - 28/06/2020                                ";
echo "=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=";
echo " Instalasi                                                       ";
...
echo " [13] Cek php.ini di PHP 7.4 (/etc/apache2/php7.4/apache/php.ini)";
echo " [14] Amankan dari PHP Shell(/etc/apache2/php7.4/apache/php.ini) ";
echo " [15] Edit php.ini (/etc/apache2/php7.4/apache/php.ini)          ";
echo "=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=";
echo " Virtualhost                                                     ";
echo " [16] Cek file virtualhost                                       ";
echo " [17] Buat file virtualhost, aktifkan, service apache2 restart   ";
...
14) read -p "Apakah anda yakin menggunakan apache server dengan PHP 7.4? y/n :" -n 1 -r
    echo  ""
    if [[ ! $REPLY =~ ^[Nn]$ ]]
    then
...
15) if [ -z "$(ls -l /etc/php/7.4/apache2/php.ini)" ]; then
    echo "File php.ini tidak ada di /etc/php/7.4/apache2/php.ini"
    else
...
echo ""
echo "Nasihosting for apache server (Ubuntu Server) - Beta 29"
