...
2) sudo apt install phpmyadmin
    ...
    sudo cp support/config.inc.php /usr/share/phpmyadmin/
    ;;
...
17) echo -n "Masukkan alamat sub domain : "
    read namasubdomain
    if [ -z "$(ls -A /etc/apache2/sites-available/$namasubdomain2.conf)" ]; then
...
18) echo -n "Masukkan alamat sub domain : "
    ...
    sudo cp support/subdomain2.conf /etc/apache2/sites-available/$namasubdomain.conf
